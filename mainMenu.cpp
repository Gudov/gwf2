...
bool GUI::isInputFullCapture() {
	return inputFullCaptureEnabled;
}

void GUI::setInputFullCapture(bool enable) {
	inputFullCaptureEnabled = enable;
}

bool GUI::isMouseEnabled() {
	return mouseEnabled;
}
void GUI::setMouseEnabled(bool enable) {
	mouseEnabled = enable;
}

bool GUI::isDemoWindow() {
	return demoWindow;
}
...
void GUI::overlayMenu() {
	...
	ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_MenuBar;
	if (corner != -1)
	...
	if (ImGui::Begin("Example: Simple overlay", &p_open, window_flags))
	{
		if (ImGui::BeginMenuBar()) {
			if (ImGui::BeginMenu("Lua")) {
				if(ImGui::MenuItem("reload 'autorun.lua'")) {
					reloadLuaInitScript();
				}
				ImGui::EndMenu();
			}
