...
// KeyBoard Options.
const int ToggleFullCaptureKey = VK_F9;
const int ToggleMouseKey = VK_F10;
...
{
	if (GetAsyncKeyState(ToggleFullCaptureKey) & 0x1) {
		GUI::setInputFullCapture(!GUI::isInputFullCapture);
	}

	if (GetAsyncKeyState(ToggleMouseKey) & 0x1) {
		GUI::setMouseEnabled(!GUI::isMouseEnabled);
	}
